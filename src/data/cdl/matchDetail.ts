...
export const getMatchDetail = async (id: number): Promise<CdlMatch> => {
  logNewline(true)
  logVerbose(`Fetching data for match CDL ${id}.`)
  ...
  const hostPlayers = matchStats && !Array.isArray(matchStats) ? matchStats.matches.hostTeam.flat(1) : []
  const awayPlayers = matchStats && !Array.isArray(matchStats) ? matchStats.matches.guestTeam.flat(1) : []
  ...
  for (const gameData of matchData.matchGamesExtended) {
    ...
    hostPlayers
      .filter((player) => player.gameMode === gameData.matchGame.mode && player.gameMap === gameData.matchGame.map)
